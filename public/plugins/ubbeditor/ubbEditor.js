function ubbEditor(_id) {
	...
	this.tToolbarSets['default'] = [ 'Source', 'separator', 'Bold', 'Italic',
			'Underline', 'RemoveFormat', 'separator', 'Link', 'Unlink',
			'Image', 'Smiley', 'separator', 'JustifyLeft', 'JustifyCenter',
			'JustifyRight', 'JustifyFull', 'separator', 'OrderedList',
			'UnorderedList', 'TextColor', 'BGColor', 'separator', 'About' ];
	this.tToolbarSets['simple'] = [ 'Bold', 'Italic', 'Underline',
			'RemoveFormat', 'separator', 'JustifyLeft', 'JustifyCenter',
			'JustifyRight', 'JustifyFull', 'separator', 'Link', 'Unlink',
			'TextColor', 'BGColor', 'separator', 'About' ];
	this.tToolbarSets['mini'] = [ 'Bold', 'Italic', 'Underline',
			'RemoveFormat', 'separator', 'Link', 'Unlink', 'TextColor',
			'BGColor', 'separator', 'About' ];
	this.tToolbarSets['mico'] = [ 'Bold', 'Italic', 'Underline',
			'RemoveFormat', 'separator', 'About' ];
	this.tToolbarSets['custom'] = [ /*'Source', 'separator',*/ 'Size', 'Bold',
			'Italic', 'Underline', 'RemoveFormat', 'separator', 'TextColor',
			'separator' ];
	...
	this.tAttachEvent = function(_obj, _eventName, _handler, _object) {
		...
		if (tobj) {
			...
			;
			try {
				tobj.addEventListener(teventName, teventHandler, false);
			} catch (e) {
				if (teventName == 'keypress')
					tobj = tobj.document;
	...
	this.tRGB2Hex = function(_strers) {
		...
		if (!tArray)
			...
			for (ti = 1; ti < tArray.length; ti++)
				tRGB2HexValue += ('0' + parseInt(tArray[ti]).toString(16))
						.slice(-2);
	...
	this.tReplace = function(_strers, _reary, _ign) {
		...
		for (var ti = 0; ti < treary.length; ti++) {
			if (!treary[ti][2])
				tstrers = tstrers.replace(treary[ti][0], (tign ? ''
						: treary[ti][1]));
		}
		;
		while (tstate1) {
	...
	this.tHTMLClear = function(_strers) {
		...
		if (tstrers) {
			...
			tstrers = tstrers
					.replace(
							/<(\/?)(script|i?frame|style|html|head|body|title|link|meta|object|\?|\%)([^>]*?)>/gi,
							'');
			tstrers = tstrers
					.replace(
							/<([a-z]+)+\s*(?:onerror|onload|onunload|onresize|onblur|onchange|onclick|ondblclick|onfocus|onkeydown|onkeypress|onkeyup|onmousemove|onmousedown|onmouseout|onmouseover|onmouseup|onselect)[^>]*>/gi,
							'<$1>');
		}
		;
		return tstrers;
			...
			var tReplaceAry = [
					...
					[
							...
							function($0, $1) {
								...
								if (tObj1.style.fontWeight.toLowerCase() == 'bold')
									tString = '[b]' + tString + '[/b]';
								if (tObj1.style.fontStyle.toLowerCase() == 'italic')
									tString = '[i]' + tString + '[/i]';
								if (tObj1.style.textDecoration.toLowerCase() == 'underline')
									tString = '[u]' + tString + '[/u]';
								if (tObj1.style.color)
									tString = '[color='
											+ tthis.tRGB2Hex(tObj1.style.color)
											+ ']' + tString + '[/color]';
								if (tObj1.style.backgroundColor)
									tString = '[hilitecolor='
											+ tthis
													.tRGB2Hex(tObj1.style.backgroundColor)
											+ ']' + tString + '[/hilitecolor]';
								return tString;
							}, true ],
					[
							...
							function($0, $1) {
								...
								if (tObj1.getAttribute('color'))
									tString = '[color='
											+ tthis.tRGB2Hex(tObj1
													.getAttribute('color'))
											+ ']' + tString + '[/color]';
								if (tObj1.style.color)
									tString = '[color='
								...
								if (tObj1.style.backgroundColor)
									tString = '[hilitecolor='
											+ tthis
													.tRGB2Hex(tObj1.style.backgroundColor)
											+ ']' + tString + '[/hilitecolor]';
								if (tObj1.getAttribute('size')) {
									tString = '[size='
											+ (tObj1.getAttribute('size') * 10)
											+ ']' + tString + '[/size]'
								}
								return tString;
							}, true ],
							...
							function($0, $1) {
								...
								if (tObj1.style.color)
									tString = '[p][color='
											...
											+ ']' + tString + '[/color][/p]';
								if (tObj1.style.backgroundColor)
									tString = '[p][hilitecolor='
											+ tthis
													.tRGB2Hex(tObj1.style.backgroundColor)
											+ ']' + tString
											+ '[/hilitecolor][/p]';
								if (tObj1.getAttribute('align'))
									tString = '[align='
											+ tObj1.getAttribute('align') + ']'
											+ tString + '[/align]';
								if (tObj1.style.textAlign)
									tString = '[align=' + tObj1.style.textAlign
											+ ']' + tString + '[/align]';
								return tString;
							}, true ],
					[
							...
							function($0, $1) {
								...
								if (tObj1.className == 'ubb_code')
									tString = '[code]' + tString + '[/code]';
								if (tObj1.className == 'ubb_quote')
									tString = '[quote]' + tString + '[/quote]';
								if (tObj1.getAttribute('align'))
									tString = '[align='
											+ tObj1.getAttribute('align') + ']'
											+ tString + '[/align]';
					...
					[
							...
							function($0, $1) {
								...
								if (tObj1.getAttribute('href'))
									tString = '[url='
											+ tObj1.getAttribute('href') + ']'
											+ tString + '[/url]';
								return tString;
							}, true ],
					[
							...
							function($0) {
								...
								if (tObj1.getAttribute('src'))
									tString = '[img]'
											+ tObj1.getAttribute('src')
											+ '[/img]';
								return tString;
							}, true ], [ /\]\[br\]\[/igm, '] [', true ],
					[ /\[br\]\[\/p\]/igm, '[/p]', true ],
	...
	this.tUBB2XHTML = function(_strers) {
		...
		if (tstrers) {
			...
			var tReplaceAry = [
					...
					[
							/\[code\]([^\[]*?)\[\/code\]/igm,
							'<div class="ubb_code" style="BORDER: #dcdcdc 1px dotted; PADDING: 5px; LINE-HEIGHT: 150%; FONT-STYLE: italic">$1</div>',
							true ],
					[
							/\[quote\]([^\[]*?)\[\/quote\]/igm,
							'<div class="ubb_quote" style="BORDER: #dcdcdc 1px dotted; PADDING: 5px; LINE-HEIGHT: 150%">$1</div>',
							true ],
					[ /\[color=([^\]]*)\]([^\[]*?)\[\/color\]/igm,
							'<font style="color: $1">$2</font>', true ],
					[
							/\[size=([^\]]*)\]([^\[]*?)\[\/size\]/igm,
							function($0, $1, $2) {
								return '<font size="' + ($1 / 10) + '">' + $2
										+ '</font>';
							}, true ],
					[ /\[hilitecolor=([^\]]*)\]([^\[]*?)\[\/hilitecolor\]/igm,
	...
	this.tcreateStyleSheet = function() {
		...
		if (!tObj1)
			tObj1 = document.getElementsByTagName('body').item(0);
		if (tObj1) {
			var tLink1 = document.createElement('link');
			tLink1.setAttribute('rel', 'stylesheet');
			tLink1.setAttribute('type', 'text/css');
			tLink1.setAttribute('href', this.tbaseURL + 'common/theme/'
					+ this.tTheme + '/css/editor.css');
	...
	this.tinsertHTML = function(_Value) {
		...
		if (tValue && this.tEditState == 1) {
			...
			try {
				...
				if (tSelectionType != 'control')
					this.tRange.pasteHTML(tValue);
				else
					this.tRange.item(0).outerHTML = tValue;
			} catch (e) {
	...
	this.tsetCommand = function(_Command, _Value) {
		...
		if (this.tEditState == 1) {
			...
			case 'SizeS':
				this.tLoadMaskClose();
				this.tRangeReselect();
				this.texecCommand('FontSize', tValue);
				break;
			case 'Italic':
			...
			case 'BGColorS':
				this.tLoadMaskClose();
				try {
					this.texecCommand('hilitecolor', tValue);
				} catch (e) {
					this.texecCommand('backcolor', tValue);
				}
				;
				break;
			case 'Link':
			...
			if (tCommand == 'Source')
				...
				this.tLoadMessage(ubbEditorLang.tError1);
		}
		;
	};
	...
	this.tShowNormal = function() {
		if (this.tEditState == 0) {
			...
			this.tEditState = 1;
			if (this.tEditUBBMode == 1)
				tnValue = this.tUBB2XHTML(tnValue);
			else
				tnValue = this.tHTMLClear(tnValue);
			tSourceImageObj = this.tEI(this.tid + '-ubbEditorToolbar-Source');
			tSourceImageObj.onmouseover = function() {
				this.className = 'ubbEditorSelected'
			};
			tSourceImageObj.onmouseout = function() {
				this.className = ''
			};
	...
	this.tLoadToolbar = function(_obj) {
		...
		if (tObj1) {
			...
			if (!tArray1)
				tArray1 = this.tToolbarSets['default'];
			for ( var tKey1 in tArray1) {
				var tnKey = tArray1[tKey1];
				if (tnKey == '-' || tnKey == 'separator')
					tHTMLString1 += '<img src="' + this.tbaseURL
							+ 'common/theme/' + this.tTheme
							+ '/images/icon/toolbar.' + tnKey + '.gif" />';
				else
					tHTMLString1 += '<img id="'
							+ this.tid
							+ '-ubbEditorToolbar-'
							...
							+ '/images/icon/'
							+ tnKey
							+ '.gif" onmouseover="this.className = \'ubbEditorSelected\';" onmouseout="this.className = \'\';" onclick="'
							+ this.tName + '.tsetCommand(\'' + tnKey
							+ '\');" />';
			}
			;
	...
	this.tLoadMaskShow = function(_strHTML) {
		...
		if (tstrHTML) {
			...
			if (tObj1) {
				...
				/*
				tObj1InnerHTML += '<div class="ubbEditorCloseTable" onclick="'
						+ this.tName + '.tLoadMaskClose();"><img src="'
						+ this.tbaseURL + 'common/theme/' + this.tTheme
						+ '/images/close.gif" /></div>';
						*/
				tObj1InnerHTML += '</div>';
				tObj1.style.display = 'none';
				tObj1.innerHTML = tObj1InnerHTML;
				tObj1.style.display = 'block';
				tObj1.style.marginLeft = (0 - Math.floor(tObj1.offsetWidth / 2))
						+ 'px';
				tObj1.style.marginTop = (0 - Math.floor(tObj1.offsetHeight / 2) + 14)
						+ 'px';
			}
		}
	};
	...
	this.tLoadAbout = function() {
		...
		tMessageTableHTML += '  </tr>';
		tMessageTableHTML += '  <tr>';
		tMessageTableHTML += '    <td class="ubbEditorTD1"><input type="button" value="'
				+ ubbEditorLang.tOK
				+ '" class="ubbEditorMessageButton" onclick="'
				+ this.tName
				+ '.tLoadMaskClose();" /></td>';
		tMessageTableHTML += '  </tr>';
	...
	this.tLoadSizeTable = function() {
		var tLinkTableHTML = '<table cellpadding="0" cellspacing="5" class="ubbEditorLinkTable">';
		tLinkTableHTML += '  <tr>';
		tLinkTableHTML += '    <td height="20" valign="top">'
				+ ubbEditorLang.tSize + '</td>';
		tLinkTableHTML += '  </tr>';
		tLinkTableHTML += '  <tr>';
		tLinkTableHTML += '    <td><input id="' + this.tid + '-ubbEditorSizeText" type="text" value="" class="ubbEditorLinkText" ondblclick="this.select();" placeholder="输入1~7的数字" /></td>';
		tLinkTableHTML += '  </tr>';
		tLinkTableHTML += '  <tr>';
		tLinkTableHTML += '    <td class="ubbEditorTD1"><input type="button" value="'
				+ ubbEditorLang.tOK
				+ '" class="ubbEditorLinkButton" onclick="'
				+ this.tName
				+ '.tsetCommand(\'SizeS\', '
				+ this.tName
				+ '.tEI(\''
				+ this.tid
				+ '-ubbEditorSizeText\').value);" />&nbsp;<input type="button" value="'
				+ ubbEditorLang.tCancel
				+ '" class="ubbEditorLinkButton" onclick="'
				+ this.tName + '.tLoadMaskClose();" /></td>';
		tLinkTableHTML += '  </tr>';
	...
	this.tLoadLinkTable = function() {
		...
		tLinkTableHTML += '  <tr>';
		tLinkTableHTML += '    <td height="20" valign="top">'
				+ ubbEditorLang.tLinkURL + '</td>';
		tLinkTableHTML += '  </tr>';
		tLinkTableHTML += '  <tr>';
		tLinkTableHTML += '    <td><input id="'
				+ this.tid
				+ '-ubbEditorLinkText" type="text" value="http://" class="ubbEditorLinkText" ondblclick="this.select();" /></td>';
		tLinkTableHTML += '  </tr>';
		tLinkTableHTML += '  <tr>';
		tLinkTableHTML += '    <td class="ubbEditorTD1"><input type="button" value="'
				+ ubbEditorLang.tOK
				...
				+ '.tEI(\''
				+ this.tid
				+ '-ubbEditorLinkText\').value);" />&nbsp;<input type="button" value="'
				+ ubbEditorLang.tCancel
				+ '" class="ubbEditorLinkButton" onclick="'
				+ this.tName + '.tLoadMaskClose();" /></td>';
		tLinkTableHTML += '  </tr>';
	...
	this.tLoadImageTable = function() {
		...
		tLinkTableHTML += '  <tr>';
		tLinkTableHTML += '    <td height="20" valign="top">'
				+ ubbEditorLang.tImageURL + '</td>';
		tLinkTableHTML += '  </tr>';
		tLinkTableHTML += '  <tr>';
		tLinkTableHTML += '    <td><input id="'
				+ this.tid
				+ '-ubbEditorImageText" type="text" value="http://" class="ubbEditorImageText" ondblclick="this.select();" /></td>';
		tLinkTableHTML += '  </tr>';
		tLinkTableHTML += '  <tr>';
		tLinkTableHTML += '    <td class="ubbEditorTD1"><input type="button" value="'
				+ ubbEditorLang.tOK
				+ '" class="ubbEditorImageButton" onclick="'
				+ this.tName
				+ '.tsetCommand(\'ImageS\', '
				+ this.tName
				+ '.tEI(\''
				+ this.tid
				+ '-ubbEditorImageText\').value);" />&nbsp;<input type="button" value="'
				+ ubbEditorLang.tCancel
				+ '" class="ubbEditorImageButton" onclick="'
				+ this.tName + '.tLoadMaskClose();" /></td>';
		tLinkTableHTML += '  </tr>';
	...
	this.tLoadSmileyTable = function() {
		...
		tSmileyTableHTML += '  <tr>';
		tSmileyTableHTML += '    <td colspan="' + tRowNum
				+ '" height="20" valign="top">'
				+ ubbEditorLang.tSmileyImage + '</td>';
		tSmileyTableHTML += '  </tr>';
		tSmileyTableHTML += '  <tr>';
		for (var ti = 0; ti < 24; ti++) {
			tni += 1;
			tSmileyTableHTML += '    <td><img src="' + this.tbaseURL
					+ 'common/theme/' + this.tTheme + '/images/smiley/'
					+ tni + '.gif" onclick="' + this.tName
					+ '.tsetCommand(\'SmileyS\', this.src);" /></td>';
	...
	this.tLoadForeColorTable = function() {
		...
		tColorTableHTML += '  <tr>';
		tColorTableHTML += '    <td colspan="' + tRowNum
				+ '" height="20" valign="top">' + ubbEditorLang.tColorPicker
				+ '</td>';
		tColorTableHTML += '  </tr>';
		tColorTableHTML += '  <tr>';
		for (var ti = 0; ti < tColorHexAryLength; ti++) {
			for (var tj = 0; tj < tColorHexAryLength; tj++) {
				for (var tk = 0; tk < tColorHexAryLength; tk++) {
					tni += 1;
					tColorTableHTML += '    <td style="BACKGROUND: #'
							+ tColorHexAry[ti] + tColorHexAry[tj]
							+ tColorHexAry[tk] + '"><img src="' + this.tbaseURL
							+ 'common/theme/' + this.tTheme
							+ '/images/space.gif" onclick="' + this.tName
							+ '.tsetCommand(\'TextColorS\', \'#'
							+ tColorHexAry[ti] + tColorHexAry[tj]
							+ tColorHexAry[tk]
							+ '\');" width="11" height="11" /></td>';
					if (tni % tRowNum == 0
							&& tni != (tColorHexAryLength * tColorHexAryLength * tColorHexAryLength)) {
						tColorTableHTML += '  </tr>';
						tColorTableHTML += '  <tr>';
					}
				}
	...
	this.toTop = function(target, parent) {
		var top = 0;
	...
	this.tInit = function(_name, _baseURL) {
		...
		if (tObj1) {
			...
			var tObj1Height = window.innerHeight - this.toTop(tObj1) - 85;
			...
			tTextarea1.style.height = (tObj1Height - this.tPanelHeight - 5)
					+ 'px';
			tTextarea1.style.maxHeight = (tObj1Height - this.tPanelHeight - 5)
					+ 'px';
			...
			tIframe1.style.height = (tObj1Height - this.tPanelHeight - 5)
					+ 'px';
			tIframe1.className = 'ubbEditorIframe';
			tDiv1.appendChild(tIframe1);
			var tObj3 = this.tFW(this.tid + '-iframe');
			if (this.tEditUBBMode == 1)
				this.tValue = this.tUBB2XHTML(this.tValue);
			var tObj3HTML = '<html><head><link href="'
					+ this.tbaseURL
					+ 'common/theme/'
					+ this.tTheme
					+ '/css/iframe.css" rel="stylesheet" type="text/css" /></head><body></body></html>';
			...
			if (this.tValue) {
				...
				if (tnValue.indexOf('<p>') != -1)
					tnValueContent = tnValue;
				else {
					var tnValueAry = tnValue.split('<br />');
					for (ti = 0; ti < tnValueAry.length; ti++)
						tnValueContent = tnValueContent + '<p>'
								+ tnValueAry[ti] + '</p>\r\n';
				}
				;
