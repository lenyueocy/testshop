...
class Qiniu
{
    ...
    public function uploadFile($filePath, $key = null, $bucket = '')
    {
        ...
        list($ret, $err) = $uploadMgr->putFile($this->uploadToken(), $key, $filePath);
        if ($err !== null) {
    ...
    {
        try {
            $filePath = Curlopt::init($url)->downloadFile(sys_get_temp_dir().'/',false,'tmp')->getResult();
            $filePath = sys_get_temp_dir().'/'.$filePath;
        } catch (Exception $e) {
            return [];
        }
    ...
    public function uploadToken($key = null,$expires = 3600)
    {
        $auth = new Auth($this->accessKey, $this->secretKey);
        $token = $auth->uploadToken($this->bucket,$key,$expires);
        return $token;
    }
