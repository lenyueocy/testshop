<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:56:"E:\workspace\B2C\template\admin\content\swiper\list.html";i:1554195608;s:43:"E:\workspace\B2C\template\admin\layout.html";i:1554195608;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="utf-8">
			...
			<form class="layui-form top-search-form" action="" onsubmit="return false;">
		        <div class="layui-form-pane">
					...
					<div class="layui-btn-group layui-layout-right">
						...
						<button class="layui-btn layui-btn-sm layui-btn-danger" id="batch-delete" data-url="<?php echo url($ctrl.'/'.$action.'_del'); ?>" data-title="轮播图">
...
<script>
var goodsList = <?php echo json_encode($goods); ?>;
...
window.recall=function(){
    ...
    tools.table({
        ...
        dealParam: {
            ...
            edit: {
                ...
                callback: function(row) {
                    tools.adminShow('编辑轮播图',"<?php echo url($ctrl.'/'.$action.'_edit'); ?>?id=" + row.id);
            ...
            del:{
                ...
                callback: function(row) {
                	tools.delItem("<?php echo url($ctrl.'/'.$action.'_del'); ?>",[row.id],'确定要删除该轮播图吗？');
                }
            }
        ...
        colModel: [{
            ...
            name: 'sign',
            dealItem:function(row){
            	//return goodsList[row.params] || '';
				if(!row.img){
                    return '<img  width="100" src="/admin/images/image-no.png">';

				}else{
                    return '<img  width="100" src="'+row.img+'">';
            	}
